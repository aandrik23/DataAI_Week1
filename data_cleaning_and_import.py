...
# Compute total_price if it's missing
def compute_total_price(row):
    if pd.isna(row['total_price']):
        if pd.notna(row['price_per_unit']) and pd.notna(row['quantity']):
            return row['price_per_unit'] * row['quantity']
        else:
            return np.nan
    return row['total_price']

df_clean['total_price'] = df_clean.apply(compute_total_price, axis=1)
